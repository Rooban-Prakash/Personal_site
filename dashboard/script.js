...
function setupWorkoutForm() {
  ...
  $('#add-exercise-btn').addEventListener('click', function () { addExerciseRow(); });

  $('#workout-form').addEventListener('submit', async function (e) {
    e.preventDefault();
    const date = $('#workout-date').value;
    const name = $('#workout-name').value.trim();
    const duration_min = parseInt($('#workout-duration').value) || null;
    if (!date || !name) return;

    const exerciseRows = $$('.exercise-row');
    const exercises = [];
    exerciseRows.forEach(function (row) {
      // const exName = row.querySelector('.ex-name input').value.trim();
      // const sets = parseInt(row.querySelector('.ex-sets input').value) || 0;
      // const reps = parseInt(row.querySelector('.ex-reps input').value) || 0;
      // const weight = parseFloat(row.querySelector('.ex-weight input').value) || null;
      const exName = row.querySelector('.ex-name input').value.trim();
      ...
      if (exName && sets > 0 && reps > 0) {
        exercises.push({ name: exName, sets: sets, reps: reps, weight_kg: weight });
      }
    });

    try {
      const workoutResult = await supabase.from('workouts').insert({ date: date, name: name, duration_min: duration_min }).select().single();
      if (workoutResult.error) throw workoutResult.error;
      const workout = workoutResult.data;

      if (exercises.length) {
        const exData = exercises.map(function (ex) { return Object.assign({}, ex, { workout_id: workout.id }); });
...
function addExerciseRow(name, sets, reps, weight) {
  ...
  row.innerHTML =
    ...
    '</label>' +
    '<label class="ex-num ex-sets">Sets' +
      '<input type="number" value="' + sets + '" min="0" placeholder="3" />' +
    '</label>' +
    '<label class="ex-num ex-reps">Reps' +
      '<input type="number" value="' + reps + '" min="0" placeholder="10" />' +
    '</label>' +
    '<label class="ex-num ex-weight">Weight (kg)' +
      '<input type="number" value="' + weight + '" step="0.5" min="0" placeholder="60" />' +
    '</label>' +
    '<button type="button" class="remove-exercise" aria-label="Remove">✕</button>';
